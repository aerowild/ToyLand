...
- **Tutor — compensation strategy:** 19+9 → 20+8 (round a near-ten addend up, adjust the other) on a number line.
- **Tutor — interactive & answer hidden:** kid taps "👉 Next step" through each step (no auto-advance); the answer stays "?" until the final step.
- **3D additive puzzles → combination cards:** bridge/hill now show predefined ≤3-addend combos (2-3 correct like 10+10+3 / 13+10, plus plausible wrong ones) via `getComboOptions`; picking builds the parts then auto-checks. (+38 tests; 89 total.)
- **Story overlays (skippable):** start-of-mission intro, portal-dive when a portal opens, and a monster-defeat line on the celebrate screen. Story beats generated by qwen3-coder (kid-safe validated) in `storyContent.json`.
- Needs real-Chrome playtest: combo-card build/win animation, BossFight visuals, portal-dive timing (3D interaction can't be verified headless).
